...
public class BoxTest {
    ...
    public void testGetBalls() {
        ...
        for (int k=0; k<TRIES_FOR_BALLS_TEST; k++) {
            ...
            assertEquals("Box.getBallsFromSmallest() did not return all the balls",
                         BOX_CAPACITY, count);
            assertEquals("The number of balls in the box was modified",
    ...
    public void testVolume() {
        box.clear();
        assertEquals("Volume of the empty Box is not zero!",
                     0, box.getVolume(), JUNIT_DOUBLE_DELTA);
        for (int i=0; i<BOX_CAPACITY; i++) {
            box.add(b[i]);
            assertEquals("Volume of the Box with "+(i+1)+" ball(s)",
    ...
    public void testSize() {
        box.clear();
        assertEquals("size() of the empty Box is not zero!", 0, box.size());
        for (int i=0; i<BOX_CAPACITY; i++) {
            box.add(b[i]);
            assertEquals("size() of the Box with "+(i+1)+" ball(s)",
                         i+1, box.size());
        }
    }
